...
{
    class SambaOpponent : APossibilityOpponent, IAIOpponent
    {
        public SambaOpponent()
            : base()
        {
        ...
        public override bool MakeShot()
        {
            ...
            while (cell_val != 0)
            {
                if (shot_count == 0)
                {
                    index = this.rand.Next(this.ProbablePlanes.Count);
                }
                shot_coor = this.ProbablePlanes[this.ProbablePlanes.Count - 1 - index].HeadShotIndex;
                cell_val = this._enemy_field.Board[shot_coor];
                index++;
            }

            shot_count++;
            this._enemy_display_board[shot_coor] = this._enemy_field.Board[shot_coor] = this.ShootEnemyHandle(new Point(shot_coor % 10, shot_coor / 10));
            ...
            APossibilityOpponent.reevaluatePossibilities(this.ProbablePlanes, this._enemy_field.Board[shot_coor], shot_coor, this._my_planes.Count, this._enemy_display_board);
